...
const GuessPage: React.FC = () => {
  ...
  // Toggle form readonly based on overwrite
  useEffect(() => {
    setIsFormReadonly(!overwrite);
  }, [overwrite]);

  // Generate tokens from actualHash
  useEffect(() => {
    if (
      actualHash &&
      actualHash !== ZERO_HASH &&
      ...
      setTokens(tokenize(actualHash, tokenSize));
    } else {
      setTokens([]);
    }
  }, [actualHash, tokenSize]);
  ...
  useEffect(() => {
    if (validateHashFormat(actualHash) && validateHashFormat(secretHash)) {
      setDummyHash(getUnrevealedHash(actualHash, secretHash));
    } else {
      setDummyHash(ZERO_HASH);
    }
  }, [actualHash, secretHash]);
  ...
  const handleGenerateHash = async (
    ...
    const generatedHash = genHashData(hashInput);
    if (type === "actual") setActualHash("0x" + removePrefix(generatedHash));
    else setSecretHash("0x" + removePrefix(generatedHash));
  ...
  const handleSubmit = async () => {
    ...
    // Minimal guess data to store
    const guessData = {
      hash: actualHash,
      tokenSize: tokenSize,
      blockIncrement: blockIncrement,
      guessType: complex ? "Complex" : paidGuess ? "Paid" : "Free",
    };
